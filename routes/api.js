...
router.get('/category/:category', (req, res) => {
  switch (req.params.category) {
    case 'All Flags':
      res.json(allFlags.map(flag => flag.name).sort());
      break;
    case 'Cities':
      res.json(cities.map(flag => flag.name).sort());
      break;
    case 'Countries':
      res.json(countries.map(flag => flag.name).sort());
      break;
    case 'Country Subdivisions':
      res.json(countrySubdivisions.map(flag => flag.name).sort());
      break;
    case 'Dependent Territories':
      res.json(dependentTerritories.map(flag => flag.name).sort());
      break;
    case 'International Organizations':
      res.json(internationalOrganizations.map(flag => flag.name).sort());
      break;
    case 'Unrecognized States':
      res.json(unrecognizedStates.map(flag => flag.name).sort());
      break;
    default:
