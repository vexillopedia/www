...
describe('api', function() {
  ...
  describe('/category/:category', function() {
    categories.forEach(category => {
      it(
        "should retrieve and sort unique flags from '" + category + "'",
        function(done) {
          chai
            .request(app)
        ...
        chai
          ...
          .end(function(err, res) {
            ...
            expect(
              flag.colors.every(
                color =>
                  ...
                  /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/.test(color.hex)
              )
            ).to.equal(true);
            ...
            if (Object.keys(flag.related).length)
              expect(
                Object.keys(flag.related).every(key =>
                  flag.related[key].every(
                    relatedFlag => typeof relatedFlag === 'string'
                  )
                )
              ).to.equal(true);
            ...
            expect(
              flag.sources.every(source => typeof source === 'string')
            ).to.equal(true);
